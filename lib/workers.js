...
workers.gatherAllChecks = () => {
	...
	_data.list('checks', (err, checks) => {
		if (!err && checks && checks.length > 0) {
			...
			debug('ErrorL could not find any checks to process');
		}
	});
};
...
workers.performCheck = originalCheckData => {
	...
	// Instaniate the request object ( using either the http or https module )
	const _moduleToUse = originalCheckData.protocol == 'http' ? http : https;
	const req = _moduleToUse.request(request, res => {
	...
	_logs.list(false, (err, logs) => {
		if (!err && logs && logs.length > 0) {
			logs.forEach(logName => {
				...
				const newFileId = id + '-' + Date.now();
				_logs.compress(id, newFileId, err => {
					if (!err) {
						// Truncate the log
						_logs.truncate(id, err => {
							if (!err) {
